...
/**
 * This class allows you to export a Map of {@link Element}s as several CSV
 * files. Three CSV files are created : nodes.csv, ways.csv, relations.csv Those
 * three CSV all contain generic informations about all elements (ID, last user
 * ID, timestamp, ..., and tags). Nodes.csv contains nodes coordinates. Ways.csv
 * contains the nodes list for each way. Relations.csv contains the members list
 * for each relation, and members roles.
 * 
 * @author Adrien PAVIE
 */
...
//OTHER METHODS
	...
	public void export(Map<String, Element> elements, File outputFolder) throws IOException {
		...
		// Create output
		StringBuilder csvNodesBuild = new StringBuilder(
		...
		StringBuilder csvRelsBuild = new StringBuilder(
		...
		for (String id : elements.keySet()) {
			...
			if (currentElem instanceof Node) {
				...
				csvWaysBuild.append(";\"[" + currentWay.getNodes().get(0).getId());
	...
	private void addInformations(StringBuilder sb, Element elem) {
		sb.append('\n' + elem.getId() + ';' + elem.getUid() + ';' + elem.getTimestamp() + ';' + elem.isVisible() + ';'
